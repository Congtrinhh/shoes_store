package db_product_detail_utils;
...
public class ProductDetailSQL {
	public static ProductGetter getProduct(String productSlug, Connection conn) throws SQLException {
		String sql = "select p.product_line_id, p.pr_slug, p.pr_name, p.pr_price, p.pr_description, min(spr_price) 'spr_price'\r\n"
		...
		if (rs.next()) {
			...
			return new ProductGetter(product_line_id, pr_slug, pr_name, pr_price, pr_description);
		}
		return null;
	}
	
	public static List<ImageGetter> getImagesOfProduct(Connection conn, String productSlug){
		String sql = "select image_id, img_file\r\n"
				+ "from product_line p join image i on p.product_line_id=i.product_line_id\r\n"
				+ "where pr_slug=?;";
		try(PreparedStatement stm = conn.prepareStatement(sql)){
			...
			while (rs.next()) {
				...
				list.add(new ImageGetter(id, base64Str));
			}
			return list;
		}
		catch(Exception e) {
